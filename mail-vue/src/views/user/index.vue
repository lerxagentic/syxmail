    ...
    <el-dialog v-model="showAdd" :title="$t('addUser')" @closed="resetAddForm">
      <div class="container">
        <el-input v-model="addForm.email" type="text" :placeholder="$t('emailAccount')" autocomplete="off">
          <template #prefix>
            <Icon @click.stop="genRandomEmail" class="gen-code" icon="bitcoin-icons:refresh-filled" width="20" height="20" :title="$t('random')"/>
          </template>
          <template #append>
...
import {generateRandomUsername} from "@/utils/random.js";
...
const domainList = computed(() => {
  const allDomains = settingStore.domainList || [];
  const roleAvail = userStore.user?.role?.availDomain;
  if (!roleAvail) return allDomains;

  let allowed = [];
  if (Array.isArray(roleAvail)) {
    allowed = roleAvail.map((d) => String(d).replace(/^@/, '').toLowerCase());
  } else if (typeof roleAvail === 'string' && roleAvail.trim()) {
    allowed = roleAvail.split(',').map((d) => d.trim().replace(/^@/, '').toLowerCase()).filter(Boolean);
  }

  if (allowed.length === 0) return allDomains;

  return allDomains.filter((item) => {
    const cleanItem = String(item).replace(/^@/, '').toLowerCase();
    return allowed.includes(cleanItem);
  });
});
...
function genRandomEmail() {
  addForm.email = generateRandomUsername(settingStore.settings.minEmailPrefix);
}

watch(domainList, (newVal) => {
  if (newVal && newVal.length > 0 && (!addForm.suffix || !newVal.includes(addForm.suffix))) {
    addForm.suffix = newVal[0];
  }
}, { immediate: true });
