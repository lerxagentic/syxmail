...
const roleService = {
	...
	hasAvailDomainPerm(availDomain, email) {
		...
		let domainArr = [];
		if (Array.isArray(availDomain)) {
			domainArr = availDomain;
		} else if (typeof availDomain === 'string') {
			domainArr = availDomain.split(',');
		}
		domainArr = domainArr.map((item) => String(item).trim().replace(/^@/, '').toLowerCase()).filter(Boolean);
		...
		return domainArr.includes(domain);
	},
